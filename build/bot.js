...
const defaultMenu = new grammy_1.Keyboard()
    ...
    .text("Отмена")
    .resized();
const foodSubcategoryKeyboard = new grammy_1.InlineKeyboard()
...
bot.hears("✅ Новый месяц", async (ctx) => {
    ...
    await ctx.reply("💰 Введи бюджет на месяц (в тенге, положительное число до 1,000,000,000):", {
        reply_markup: new grammy_1.Keyboard().text("Отмена").resized(),
    });
});
bot.hears("➕ Добавить расход", async (ctx) => {
    ctx.session.addingExpense = true;
    await ctx.reply("✏️ Введи расход(ы):\nПример:\nтатарка 50000\nили\nтатарка - ужин 50000\n\n‼ По очереди расходы добавлять. (По одному)", { reply_markup: new grammy_1.Keyboard().text("Отмена").resized() });
});
bot.hears("🗑️ Удалить расход", async (ctx) => {
    ...
    for (const e of expenses) {
        ...
        const label = `${getEmoji(e.category)} ${e.category}${subcategory} — ${e.amount}₸${e.comment ? ` (${e.comment})` : ""}`;
        const keyboard = new grammy_1.InlineKeyboard().text("❌ Удалить", `delete_${e.id}`);
        await ctx.reply(`🧾 ${label}`, { reply_markup: keyboard });
    }
});
bot.callbackQuery(/^delete_([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})$/, async (ctx) => {
    try {
        ...
        if (selectError)
            throw selectError;
        if ((expense === null || expense === void 0 ? void 0 : expense.user_id) !== userId) {
...
bot.callbackQuery(/^food_(.+)$/, async (ctx) => {
    try {
        ...
        const subcategory = action === "yandex" ? "Яндекс" : action === "small" ? "Small" : "Продуктовый";
        ...
        await ctx.reply(`✅ Добавлено:\n• ${getEmoji(category)} ${category}${subcategoryText} — ${amount} ₸${comment ? ` (${comment})` : ""}`, { reply_markup: defaultMenu });
...
bot.hears("📊 Посчитать расходы", async (ctx) => {
    ...
    const list = (expenses === null || expenses === void 0 ? void 0 : expenses.length)
        ? expenses
            .map(e => {
...
bot.hears("💡 Советы по экономии", async (ctx) => {
    ...
    ctx.reply("💾 Сколько ты хочешь сохранить в этом месяце? (в тенге, положительное число до 1,000,000,000)", {
        reply_markup: new grammy_1.Keyboard().text("Отмена").resized(),
    });
});
...
bot.on("message:text", async (ctx) => {
    ...
    if (ctx.session.confirmResetStep === 1 && text.toLowerCase() === "да") {
        ...
        return ctx.reply("🛑 Подтверди ещё раз: сбросить все данные за месяц? Напиши `Да` повторно.", { reply_markup: new grammy_1.Keyboard().text("Отмена").resized() });
    }
    ...
    if (ctx.session.addingExpense) {
        ...
        const failed = lines
            ...
            .filter(item => !item.parsed);
        if (results.length === 0) {
            const errorMsg = "❌ Не удалось распознать расходы. Используйте формат:\n" +
                "еда 5000\nили\nеда - ужин 5000" +
                (failed.length > 0
                    ? `\n\nОшибочные строки:\n${failed
        ...
        try {
            ...
            const successMsg = `✅ Добавлено:\n` +
                `• ${getEmoji(expense.category)} ${expense.category} — ${expense.amount} ₸${expense.comment ? ` (${expense.comment})` : ""}` +
                (failed.length > 0
